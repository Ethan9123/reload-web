...
(function () {
  ...
  const LANG = {
    zh: {
      ...
      "map.arcadia": "Arcadia 阿卡迪亚 (3-4)", "map.imperial": "帝国皇朝 (2-3)", "map.transit": "转运站 · 4传送门 (4-6)", "map.ring": "环形争霸战 (4-6)", "map.metropolis": "大都会 · 迷宫 (3-4)", "map.reactor": "反应炉 · 太阳能 (4-6)", "map.crossfire": "十字火力 · 迷宫+太阳能 (4-6)",
    ...
    en: {
      ...
      "map.arcadia": "Arcadia (3-4)", "map.imperial": "Imperial Dynasty (2-3)", "map.transit": "Transit Hub · 4 portals (4-6)", "map.ring": "Ring Arena (4-6)", "map.metropolis": "Metropolis · Maze (3-4)", "map.reactor": "Reactor · Solar (4-6)", "map.crossfire": "Crossfire · Maze+Solar (4-6)",
    ...
    fr: {
      ...
      "map.arcadia": "Arcadia (3-4)", "map.imperial": "Dynastie impériale (2-3)", "map.transit": "Hub de transit · 4 portails (4-6)", "map.ring": "Arène circulaire (4-6)", "map.metropolis": "Métropole · Labyrinthe (3-4)", "map.reactor": "Réacteur · Solaire (4-6)", "map.crossfire": "Tir croisé · Labyrinthe+Solaire (4-6)",
    ...
    es: {
      ...
      "map.arcadia": "Arcadia (3-4)", "map.imperial": "Dinastía imperial (2-3)", "map.transit": "Centro de tránsito · 4 portales (4-6)", "map.ring": "Arena circular (4-6)", "map.metropolis": "Metrópolis · Laberinto (3-4)", "map.reactor": "Reactor · Solar (4-6)", "map.crossfire": "Fuego cruzado · Laberinto+Solar (4-6)",
  ...
  function fameTrackHTML(p) {
    ...
    for (const k of FAME_ORDER) {
      ...
      segs += `<span class="ft-seg" data-w="${w}" style="width:${startW}%;background:${FAME_COLOR[k] || "#888"}" title="${(D.FAME[k] && D.FAME[k].name) || k} ×${c} = ${c * val}"></span>`;
    }
  ...
  // after renderPlayers rebuilds the cards, ease each fame segment from its old width to its data-w target
  function animateFameTracks() {
    requestAnimationFrame(() => {
      document.querySelectorAll("#players-area .fametrack .ft-seg").forEach(s => {
        const w = s.getAttribute("data-w"); if (w != null) s.style.width = w + "%";
      });
    });
  }
